...
public class DriveService {
    ...
    public File findFileByName(String filename) throws IOException, GeneralSecurityException {
        // Query Google Drive for the file using the filename
        // You need to implement this method based on Google Drive API
        // Refer to Google Drive API documentation on how to search for files
        Drive driveInstance = googleDriveManager.getInstance();
        FileList filelist = driveInstance.files().list().setQ("name='" + filename + "'").execute();

        return filelist.getFiles().get(0);
    }

    public byte[] getFileStream(File existedFile) throws IOException, GeneralSecurityException {
        // Retrieve the file content stream using Google Drive API
        Drive driveInstance = googleDriveManager.getInstance();
        InputStream data = driveInstance.files().get(existedFile.getId()).executeMediaAsInputStream();
